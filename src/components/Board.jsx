...
// Adaptar estado nuevo (pilas numeradas) al formato que espera este Board
function adaptState(s) {
  if (!s || !s.houses) return s;
  // Convertir houses objeto {1:[],2:[]...} a array de 8 elementos
  const housesArr = HOUSE_PILES.map(id => s.houses[id] || []);
  // Convertir foundations numeradas a formato {suit_owner: []}
  const foundations = {};
  const suitMap = { P: 'spades', C: 'hearts', D: 'diamonds', T: 'clubs' };
  const owners = ['human', 'ai'];
  // Cada palo tiene 2 fundaciones — una para cada owner
  Object.entries(SUIT_FOUNDATIONS).forEach(([suit, [idA, idB]]) => {
    const suitName = suitMap[suit];
    foundations[suitName + '_human'] = s.foundations[idA] || [];
    foundations[suitName + '_ai']    = s.foundations[idB] || [];
  });
  // Adaptar flipped -> flippedCard
  const human = s.human ? { ...s.human, flippedCard: s.human.flipped ?? s.human.flippedCard ?? null } : s.human;
  const ai    = s.ai    ? { ...s.ai,    flippedCard: s.ai.flipped    ?? s.ai.flippedCard    ?? null } : s.ai;
  // Adaptar suit de P/C/D/T a spades/hearts/diamonds/clubs para render
  const adaptCards = (arr) => (arr || []).map(c => ({
    ...c,
    suit: suitMap[c.suit] || c.suit,
    color: c.color || ((['C','D'].includes(c.suit) || ['hearts','diamonds'].includes(c.suit)) ? 'red' : 'black'),
    faceUp: c.faceUp ?? false,
  }));
  const adaptPile = (arr) => adaptCards(arr || []);
  const adaptPlayer = (p) => p ? {
    ...p,
    crapette: adaptPile(p.crapette),
    talon: adaptPile(p.talon),
    discard: adaptPile(p.discard),
    flippedCard: p.flipped ? { ...p.flipped, suit: suitMap[p.flipped.suit] || p.flipped.suit } : (p.flippedCard || null),
  } : p;

  return {
    ...s,
    houses: housesArr.map(pile => adaptPile(pile)),
    foundations: Object.fromEntries(
      Object.entries(foundations).map(([k, v]) => [k, adaptPile(v)])
    ),
...
export function Board({ config, onReset, onDashboard, onExit }) {
  ...
  const checkStopOnSelect = (card, source) => {
    ...
    const mandatory = getMandatoryMoves(state.human, rawState.houses, rawState.foundations, state.aiLevel, true);
